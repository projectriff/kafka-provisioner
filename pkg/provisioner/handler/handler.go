...
func (creationHandler *TopicCreationRequestHandler) GetHandlerFunc() http.HandlerFunc {
	return func(responseWriter http.ResponseWriter, request *http.Request) {
		...
		topicExists, kafkaError := creationHandler.KafkaClient.TopicExists(topicName)
		if kafkaError != nil {
			responseWriter.WriteHeader(http.StatusInternalServerError)
			if err := kafkaError.GeneralError; err != nil {
				_, _ = fmt.Fprintf(creationHandler.Writer, "Error trying to list topics to see if %q exists: %v\n", topicName, err)
		...
		if !topicExists {
			if err := creationHandler.KafkaClient.CreateTopic(topicName); err != nil {
				responseWriter.WriteHeader(http.StatusInternalServerError)
				_, _ = fmt.Fprintf(creationHandler.Writer, "Error creating topic %q: %v\n", topicName, err)
		...
		if err := encodeResponse(responseWriter, creationHandler.Gateway, topicName); err != nil {
			_, _ = fmt.Fprintf(creationHandler.Writer, "Failed to write json response: %v", err)
			return
		}
		_, _ = fmt.Fprintf(creationHandler.Writer, "Reported successful topic %q\n", topicName)
	}
}
